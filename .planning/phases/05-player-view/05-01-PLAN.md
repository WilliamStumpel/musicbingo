...
<task type="auto">
  <name>Task 4: Add song duration timer to host view</name>
  <files>musicbingo_host/src/components/SongTimer.jsx, musicbingo_host/src/components/SongTimer.css, musicbingo_host/src/pages/HostView.jsx</files>
  <action>
Create SongTimer component for DJ to track song playback duration:

Features:
- Shows elapsed time since current song marked "now playing"
- Configurable target duration (default: 30 seconds)
- Visual flash when target reached (no audio alert)
- Resets automatically when song changes

Layout:
- Display: "0:00 / 0:30" format (elapsed / target)
- Position: Near "Now Playing" section in host view
- Progress bar underneath showing percentage

Visual feedback at target:
- Timer container flashes (amber/yellow pulse animation)
- Text changes color to indicate "time to switch"
- Animation continues until song changes
- CSS animation only, no sound

Props:
- nowPlaying: current song ID (null = timer stops/resets)
- targetSeconds: configurable target (default 30)

Implementation:
- useEffect with setInterval (1 second tick)
- Clear interval when nowPlaying changes or unmounts
- Track startTime when nowPlaying changes
- Calculate elapsed = now - startTime
  </action>
  <verify>Timer counts up when song is playing, flashes at 30 seconds, resets on song change</verify>
  <done>Song duration timer helps DJ know when to switch songs in external player</done>
</task>
...
- [ ] Song timer shows elapsed time and flashes at target duration
</verification>
